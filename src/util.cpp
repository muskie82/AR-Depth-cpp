...
cv::Mat View::GetImage(std::string image_folder, bool resize) {
  ...
  assert(img.data);
  if (resize) cv::resize(img, img, cv::Size(width_resize, height_resize));
  return img;
}
...
std::vector<int> Reconstruction::GetReferenceFrames(int view_id) {
  ...
  std::vector<int> ref;
  if (kf.first == -1 and kf.second == -1) return ref;
  double dist =
      (views[kf.second].Position() - views[kf.first].Position()).norm() / 2;
...
cv::Mat Reconstruction::GetSparseDepthMap(int frame_id, bool resize) {
  ...
  cv::Mat depth_map =
      cv::Mat::zeros(cv::Size(camera.width, camera.height), CV_64FC1);
