...
cv::Mat ARDepth::GetGradientMagnitude(const cv::Mat& img_grad_x,
...
std::pair<cv::Mat, cv::Mat> ARDepth::GetFlowGradientMagnitude(
    const cv::Mat& flow, const cv::Mat& img_grad_x, const cv::Mat& img_grad_y) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 1; x < width; x++) {
      Eigen::Vector2d gradient_dir(img_grad_y.at<double>(y, x),
      ...
      if (p0[0] < 0 or p1[0] < 0 or p0[1] < 0 or p1[1] < 0 or p0[0] >= height or
          p0[1] >= width or p1[0] >= height or p1[1] >= height) {
        reliability.at<float>(y, x) = -100;
        continue;
      }

      Eigen::Vector2d flow_p0(flow.at<cv::Vec2f>(int(p0[0]), int(p0[1]))[0],
                              flow.at<cv::Vec2f>(int(p0[0]), int(p0[1]))[1]);
      Eigen::Vector2d flow_p1(flow.at<cv::Vec2f>(int(p1[0]), int(p1[1]))[0],
...
cv::Mat ARDepth::GetSoftEdges(const cv::Mat& image,
                              const std::vector<cv::Mat>& flows) {
  std::pair<cv::Mat, cv::Mat> img_grad = GetImageGradient(image);
  cv::Mat img_grad_magnitude =
      GetGradientMagnitude(img_grad.first, img_grad.second);
  ...
  for (const auto& flow : flows) {
    std::pair<cv::Mat, cv::Mat> FlowGradMag =
        GetFlowGradientMagnitude(flow, img_grad.first, img_grad.second);
  ...
  for (int y = 1; y < height; y++) {
    for (int x = 1; x < width; x++) {
      ...
      if (ay < tg22x) {
        ...
        if (m > mag.at<double>(y - 1, x + 1) and
            m >= mag.at<double>(y + 1, x - 1)) {
          if (m > tau_high and soft_edges.at<double>(y, x) > tau_flow) {
            seeds.push(std::make_pair(x, y));
            edges.at<double>(y, x) = 255;
          } else if (m > tau_low) {
...
cv::Mat ARDepth::GetInitialization(const cv::Mat& sparse_points,
...
cv::Mat ARDepth::DensifyFrame(const cv::Mat& sparse_points,
    ...
    for (int x = 1; x < w - 1; x++) {
      ...
      if (sparse_points.at<double>(y, x) > 0.00) {
        tripletList.emplace_back(
            Eigen::Triplet<double>(num_entries, idx, lambda_d));
        b(num_entries) = (1.0 / sparse_points.at<double>(y, x)) * lambda_d;
        num_entries++;
      } else if (!last_depth_map.empty() and
                 last_depth_map.at<double>(y, x) > 0) {
        tripletList.emplace_back(
            Eigen::Triplet<double>(num_entries, idx, lambda_t));
        b(num_entries) = (1.0 / last_depth_map.at<double>(y, x)) * lambda_t;
        num_entries++;
      }

      double smoothnes_weight =
          lambda_s * std::min(smoothness.at<double>(y, x),
      ...
      if (hard_edges.at<double>(y, x) == hard_edges.at<double>(y - 1, x)) {
        ...
        tripletList.emplace_back(
            Eigen::Triplet<double>(num_entries, idx - w, smoothnes_weight));
        tripletList.emplace_back(
            Eigen::Triplet<double>(num_entries, idx, -smoothnes_weight));
        b(num_entries) = 0;
        num_entries++;
      }

      smoothnes_weight = lambda_s * std::min(smoothness.at<double>(y, x),
      ...
      if (hard_edges.at<double>(y, x) == hard_edges.at<double>(y, x - 1)) {
        ...
        tripletList.emplace_back(
            Eigen::Triplet<double>(num_entries, idx - 1, smoothnes_weight));
        tripletList.emplace_back(
            Eigen::Triplet<double>(num_entries, idx, -smoothnes_weight));
        b(num_entries) = 0;
        num_entries++;
...
void ARDepth::visualizeImg(const cv::Mat& raw_img, const cv::Mat& raw_depth,
  ...
  // Sparse depth map
  cv::Mat raw_depth_visual =
      cv::Mat::zeros(raw_depth.size(), raw_depth.depth());
  cv::Mat tmp = cv::Mat::ones(raw_depth.size(), raw_depth.depth()) * 255;
  tmp.copyTo(raw_depth_visual, raw_depth > 0);
  cv::resize(raw_depth_visual, raw_depth_visual,
             cv::Size(width_visualize, height_visualize), 0, 0, cv::INTER_AREA);

  // Dense depth map
  double min_val, max_val;
  cv::Mat filtered_depthmap_visual =
      cv::Mat::zeros(filtered_depth.size(), filtered_depth.depth());
  ...
  filtered_depthmap_visual =
      255 * (filtered_depthmap_visual - min_val) / (max_val - min_val);
  filtered_depthmap_visual.convertTo(filtered_depthmap_visual, CV_8U);
  cv::applyColorMap(filtered_depthmap_visual, filtered_depthmap_visual,
                    2);  // COLORMAP_JET
  cv::resize(filtered_depthmap_visual, filtered_depthmap_visual,
             cv::Size(width_visualize, height_visualize));
...
void ARDepth::run() {
  ...
  int skip_frames =
      recon
          .GetNeighboringKeyframes(
              recon.GetNeighboringKeyframes(recon.ViewIds()[15]).first)
          .second;

  std::deque<cv::Mat> last_depths;
  cv::Mat last_depth;

  std::cout << "Using the first " << skip_frames
            << " frames to initialize (these won't be saved)." << std::endl;
  int count = 0;
  for (const auto& frame : recon.ViewIds()) {
    std::vector<int> reference_frames = recon.GetReferenceFrames(frame);
    if (reference_frames.empty()) continue;

    std::cout << "==> Processing frame " << recon.views[frame].name
              << std::endl;
    ...
    if (!recon.views[frame].isKeyframe()) {
      std::pair<int, int> neiboring_keyframes =
          recon.GetNeighboringKeyframes(frame);
      assert(neiboring_keyframes.first != -1 and
             neiboring_keyframes.second != -1);
      last_keyframe = neiboring_keyframes.first;
    }

    cv::Mat depth = DensifyFrame(recon.GetSparseDepthMap(last_keyframe, resize),
                                 edges, soft_edges, last_depth);
    last_depths.push_back(depth);
    if (last_depths.size() > k_T) last_depths.pop_front();
