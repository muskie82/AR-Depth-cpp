...
size_t ColmapReader::split(const std::string &txt,
...
std::map<int, Camera> ColmapReader::ReadColmapCamera(std::string filename) {
  ...
  std::string str;
  if (ifs.fail()) {
    throw std::invalid_argument("Failed to read camera file");
  }
...
std::map<int, View> ColmapReader::ReadColmapImages(std::string filename) {
  ...
  while (getline(ifs, str)) {
    ...
    views[id_value].orientation =
        Eigen::Quaterniond(std::stod(tokens[1]), std::stod(tokens[2]),
                           std::stod(tokens[3]), std::stod(tokens[4]));
    views[id_value].translation = Eigen::Vector3d(
    ...
    for (int idx = 0; idx < tokens.size() / 3; idx++) {
      ...
      views[id_value].points2d[point_id] = std::make_pair(
          std::stod(tokens[idx * 3 + 0]), std::stod(tokens[idx * 3 + 1]));
    }
  };
...
std::map<int, Point> ColmapReader::ReadColmapPoints(std::string filename) {
  ...
  std::string str;
  if (ifs.fail()) {
    throw std::invalid_argument("Failed to read point file");
  }
  ...
  while (getline(ifs, str)) {
    ...
    points[id_value].position3d = Eigen::Vector3d(
...
Reconstruction ColmapReader::ReadColmap(const std::string &poses_folder,
